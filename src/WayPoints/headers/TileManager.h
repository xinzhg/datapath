...
class ChunkMetaData {
	...
	typedef map<__uint64_t, multimap<vector<ChunkID>, vector<ChunkID> > > BucketToFloorMap;
	...
	typedef  map<int, set<ChunkID> > BucketToChunkMap;
	...
	typedef  map<__uint64_t, set<ChunkID> > BucketToChunkMapSmall;
...
public:
	...
	bool GetTile (__uint64_t& bucket, vector<ChunkID>& lhs, vector<ChunkID>& rhs);
	// get the floor and also bucket number
	bool GetFloor (__uint64_t& bucket, vector<ChunkID>& lhs, vector<ChunkID>& rhs);
	// get the slice of the floor and also bucket number
	bool GetSlice (__uint64_t& bucket, vector<ChunkID>& lhs, vector<ChunkID>& rhs);
	// using bucket, give me all tiles belonging to that bucket
	multimap<vector<ChunkID>, vector<ChunkID> > GetAllTiles (__uint64_t bucket);

	// if tile is returned, add it into the floor to send back again at later time
	void UnprocessedTile(__uint64_t bucketID, vector<ChunkID> chunkLHS, vector<ChunkID> chunkRHS);
	void UnprocessedFloor(__uint64_t bucketID, set<ChunkID> chunkLHS, set<ChunkID> chunkRHS);
	void UnprocessedSlice(__uint64_t bucketID, vector<ChunkID> chunkLHS, vector<ChunkID> chunkRHS);
	...
	void PrintAllTiles ();
	void PrintAllSlices ();
	void PrintFloor (map<ChunkID, set<ChunkID> >& floor);

private:

	// helper function
	void Insert (int bucket, ChunkID chunkID, map<int, set<ChunkID> >& ChunksToDisk);
