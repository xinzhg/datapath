...
void Iterator :: swap (Iterator& swapMe) {
	...
	SWAP(myData, swapMe.myData);
	SWAP(bytesToRequest, swapMe.bytesToRequest);
	SWAP(curPosInColumn, swapMe.curPosInColumn);
	SWAP(firstInvalidByte, swapMe.firstInvalidByte);
	SWAP(objLen, swapMe.objLen);
	SWAP(colLength, swapMe.colLength);
	SWAP(isWriteOnly, swapMe.isWriteOnly);
	SWAP(myMinByteToGetLength, swapMe.myMinByteToGetLength);
	SWAP(isInValid, swapMe.isInValid);
}
