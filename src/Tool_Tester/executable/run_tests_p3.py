...
# Python 3 version of run_tests.sh if that ends up being too slow
...
def HandleFile( fileName ):
    ...
    try:
        bStuff = subprocess.check_output([M4, "-DFILE_NAME=" + fileName, M4_DATAFILE])
        stuff = bStuff.decode(sys.stdout.encoding)
    except:
    ...
    try:
        ...
        rezFile = REZ_FILE.decode(sys.stdout.encoding)
    except:
        print( "Error getting result file name from", fileName )
        m4error = True

    if not m4error:
        CUR_RESULTS_FILE = os.path.join(RESULTS_FOLDER , rezFile)
