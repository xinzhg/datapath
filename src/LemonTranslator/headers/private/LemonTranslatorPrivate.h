...
// Graph is made of WayPointID, so keep WayPointID map for each node
//ListDigraph::NodeMap<WayPointID> nodeMap;

map<WayPointID, ListDigraph::Node> IDToNode;
...
map<ListDigraph::Node, LT_Waypoint*> nodeToWaypointData;
...
map<QueryID, ListDigraph::Node> queryToRootMap;
...
bool GetWaypointAttr(WayPointID WPID, SlotContainer& atts, set<SlotID>& attr, LT_Waypoint*& WP);
