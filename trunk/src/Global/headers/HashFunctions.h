...
/*
template <typename T>
inline __uint64_t Hash(const T& x, __uint64_t b = H_b){
	if (sizeof(T)==4)
		return CongruentHash( (__uint32_t&)x, H_b);
	if (sizeof(T)==8)
		return CongruentHash( (__uint64_t&)x, H_b);

	if( sizeof(T) % 8 == 0 ) {
	  // Interpret the input as an array of uint64
	  __uint64_t * xArr = (__uint64_t *) &x;
	  __uint64_t xArr_size = sizeof(T) / 8;

	  __uint64_t prev = b;

	  for (__uint64_t i=0; i<xArr_size; i++) {
	    prev = CongruentHash( xArr[i], prev );
	  }
	...
	if( sizeof(T) % 4 == 0 ) {
	  // Interpret the input as an array of uint32
	  __uint32_t * xArr = (__uint32_t *) &x;
	  __uint64_t xArr_size = sizeof(T) / 4;
	...
	if( sizeof(T) % 2 == 0 ) {
	  // Interpret the input as an array of uint16
	  __uint16_t * xArr = (__uint16_t *) &x;
	  __uint64_t xArr_size = sizeof(T) / 2;
	...
	// Interpret the input as an array of uint8
	// Worst case
	__uint8_t * xArr = (__uint8_t *) &x;
	__uint64_t xArr_size = sizeof(T);
