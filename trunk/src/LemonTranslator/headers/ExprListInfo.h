...
#ifdef DTM_DEBUG
      // Debugging output to help figure out what exactly is going on during
      // the parsing of strings and string literals.
      /* if( list[i].type == "HString" || list[i].type == "STRING_LITERAL" ) { */
      /*  cout << list[i].type << " " << list[i].sExpr << " - isCt: " << list[i].isCT << " noExtract: " */
      /* 	   << args[i].forceNoExtract() << endl; */
      //}
#endif
