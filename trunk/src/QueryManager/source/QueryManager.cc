...
QueryID QueryManager::GetQueryID(string queryName) {
	...
	if(qWrapper != NULL)
	{
		//get the bit indexqueryMe
		rez=qWrapper->myID;
	}
	else {
		WARNING("We got asked to get id of a non-existing query %s!", queryName.c_str());
	}
	// return mutex
	pthread_mutex_unlock (&queryManagerMutex);
	return rez;
...
void QueryManager::GenerateM4Files(string fullFileName) {
	//get the mutex
	pthread_mutex_lock (&queryManagerMutex);

	fstream queriesM4;
	//open the m4 file to be created
	queriesM4.open(fullFileName.c_str(), fstream::out | fstream::trunc);

	string m4_define = "m4_define";
	QueriesNameInfoMap::iterator mapItr;

	// Loop through all the queries in the system
	for(mapItr = mapQueriesNameInfo.begin();  mapItr != mapQueriesNameInfo.end(); mapItr++)
	{
		//retrieve the query information
		string queryNm = (*mapItr).first;
		QueryWrapper* queryInfo = (*mapItr).second;

		//write it into m4 file
		queriesM4 << m4_define << "(" <<  "QID_" << queryNm << "," << queryInfo->myMap  << ")" << endl;
