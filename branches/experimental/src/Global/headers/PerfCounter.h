...
PerfCounter :: PerfCounter(EventType evType, bool global /* if true, PC per process */){
    ...
    WARNINGIF(file == 0, "Performance counter could not be started. It will return 1 for all calls");
#endif
  }
...
#if 0
      printf("RAW Counter: %ld %ld %ld\n", count.value, count.enabled, count.running);
#endif
      if ( ret == sizeof(uint64_t))
	return count.value;
      else if (ret == sizeof(count)){
	if (count.running>0 && count.running < count.enabled){ /* we need to scale the counter */
	  return (uint64_t)((double)count.value * count.enabled / count.running + 0.5);
	}
	return count.value; /* not scaled */
      }
    }
    return 1;
#else
