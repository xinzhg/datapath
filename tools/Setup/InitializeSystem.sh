...
echo ""
echo "Where shold the metadata file be located (complete path)?"
echo -n "Please use the .sqlite extension [$DEFAULT_METAFILE]: "
...
if [ -s $META_FILE ]; then
	echo -n "The file $META_FILE already exists. Are you sure you want to delete it? (Yes/No): "
	read
	if [ "$REPLY" != "Yes" ]; then
		echo "Sorry, cannot proceed with an existing metadata file."
		exit -1
	fi
